...
ApplicationMain.preloader_onComplete = function(event) {
	...
	var hasMain = false;
	var _g = 0;
	var _g1 = Type.getClassFields(BunnyMark);
	while(_g < _g1.length) {
	...
	if(hasMain) Reflect.callMethod(BunnyMark,Reflect.field(BunnyMark,"main"),[]); else {
...
var BunnyMark = function() {
	...
	Env.setup();
	openfl.Lib.current.set_scaleX(openfl.Lib.current.set_scaleY(Env.screenDensity));
	...
	this.addChild(new tests.TilesheetTest());
...
BunnyMark.prototype = $extend(openfl.display.Sprite.prototype,{
	fps_onClick: function(event) {
		this.stage.frameRate = 90 - this.stage.frameRate;
	}
...
Background.prototype = $extend(openfl.display.Sprite.prototype,{
	...
	,paint: function() {
		...
		this.get_graphics().drawRect(0,0,this.stage.stageWidth * 1.2,this.stage.stageHeight * 1.2);
...
$hxClasses["Env"] = Env;
Env.__name__ = ["Env"];
Env.screenDensity = null;
Env.width = null;
Env.height = null;
Env.setup = function() {
	Env.screenDensity = 1;
	Env.stage_onResize(null);
	openfl.Lib.current.stage.addEventListener(openfl.events.Event.RESIZE,Env.stage_onResize);
};
Env.stage_onResize = function(event) {
	Env.width = Math.ceil(openfl.Lib.current.stage.stageWidth / Env.screenDensity);
	Env.height = Math.ceil(openfl.Lib.current.stage.stageHeight / Env.screenDensity);
};
...
tests.TilesheetTest = function() {
	...
	this.tilesheet = new openfl.display.Tilesheet(this.bunnyAsset);
	this.tilesheet.addTileRect(new openfl.geom.Rectangle(0,0,this.bunnyAsset.width,this.bunnyAsset.height));
	var bunny;
	var _g1 = 0;
	...
	openfl.Lib.current.stage.addEventListener(openfl.events.Event.RESIZE,$bind(this,this.stage_resize));
	this.stage_resize(null);
};
$hxClasses["tests.TilesheetTest"] = tests.TilesheetTest;
tests.TilesheetTest.__name__ = ["tests","TilesheetTest"];
tests.TilesheetTest.__super__ = openfl.display.Sprite;
tests.TilesheetTest.prototype = $extend(openfl.display.Sprite.prototype,{
	...
	,enterFrame: function(e) {
		...
		while(_g1 < _g) {
			...
			this.drawList[index] = bunny.position.x;
			this.drawList[index + 1] = bunny.position.y;
			this.drawList[index + 3] = bunny.scale;
			this.drawList[index + 4] = bunny.rotation;
			this.drawList[index + 5] = bunny.alpha;
		}
		this.tilesheet.drawTiles(this.get_graphics(),this.drawList,this.smooth,11);
