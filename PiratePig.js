...
ApplicationMain.embed = $hx_exports.openfl.embed = function(elementName,width,height,background) {
	...
	var id;
	var image = new Image();
	id = "images/applejack_clapping.gif";
	...
	if(ApplicationMain.total == 0) ApplicationMain.start(); else {
		...
		while(_g < sounds.length) {
			...
			++_g;
			var sound19 = new openfl.media.Sound();
			sound19.addEventListener(openfl.events.Event.COMPLETE,ApplicationMain.sound_onComplete);
			sound19.addEventListener(openfl.events.IOErrorEvent.IO_ERROR,ApplicationMain.sound_onIOError);
			sound19.load(new openfl.net.URLRequest(soundName + ".ogg"));
		}
	}
};
...
ApplicationMain.preloader_onComplete = function(event) {
	...
	var hasMain = false;
	var _g = 0;
	var _g1 = Type.getClassFields(piratepig.PiratePig);
	while(_g < _g1.length) {
	...
	if(hasMain) Reflect.callMethod(piratepig.PiratePig,Reflect.field(piratepig.PiratePig,"main"),[]); else {
		var instance = Type.createInstance(DocumentClass,[]);
		if(js.Boot.__instanceof(instance,openfl.display.DisplayObject)) openfl.Lib.current.addChild(instance); else {
			haxe.Log.trace("Error: No entry point found",{ fileName : "ApplicationMain.hx", lineNumber : 488, className : "ApplicationMain", methodName : "preloader_onComplete"});
			haxe.Log.trace("If you are using DCE with a static main, you may need to @:keep the function",{ fileName : "ApplicationMain.hx", lineNumber : 489, className : "ApplicationMain", methodName : "preloader_onComplete"});
		}
	}
};
...
var __ASSET__fonts_freebooterupdated_ttf = function() {
	...
	this.fontName = "fonts/FreebooterUpdated.ttf";
};
...
piratepig.PiratePigGame.NUM_COLUMNS = 12;
piratepig.PiratePigGame.NUM_ROWS = 12;
piratepig.PiratePigGame.tileImages = ["images/applejack_clapping.gif","images/psychoticmushrooom.gif","images/ispanol.png","images/dd.gif","images/pony.png","images/Red_Eyes_Black_Dragon_Avatar_by_PhuiJL.gif"];
